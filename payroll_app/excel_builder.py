...
# from payroll_app.config import * # Uses your HEADER_FILL, RELAY_FILL, etc.
...
#             # Formatting: Hide zeros
#             if val == 0:
#                 cell.number_format = ';;;' # Invisible if 0
            
#             # Color Coding Base Columns
...
#         col_ptr = len(final_df.columns) + 1
        
#         # --- FORMULA SECTION ---
        
#         # 2. Weekly Relay Loads
#         w1_relay_range = f"{gcl(relay_display[0])}{row_num}:{gcl(relay_display[min(6, len(relay_display)-1)])}{row_num}"
#         ws.cell(row_idx, col_ptr).value = f'=COUNTIF({w1_relay_range}, ">0")'
        
#         w2_val = "0"
#         if len(relay_display) > 7:
#             w2_relay_range = f"{gcl(relay_display[7])}{row_num}:{gcl(relay_display[-1])}{row_num}"
#             w2_val = f'=COUNTIF({w2_relay_range}, ">0")'
#         ws.cell(row_idx, col_ptr + 1).value = w2_val
#         ws.cell(row_idx, col_ptr + 2).value = f"={gcl('W1_Relay_Loads')}{row_num}+{gcl('W2_Relay_Loads')}{row_num}"
#         col_ptr += 3

#         # 3. ADP Week 1 & 2
#         w1_adp_range = f"{gcl(adp_display[0])}{row_num}:{gcl(adp_display[min(6, len(adp_display)-1)])}{row_num}"
...
#             w2_adp_range = f"{gcl(adp_display[7])}{row_num}:{gcl(adp_display[-1])}{row_num}"
...
#         # 4. Totals
#         ws.cell(row_idx, col_ptr).value = f"={gcl('W1 Hours')}{row_num}+{gcl('W2 Hours')}{row_num}" # Total ADP Hours
...
#         # 5. Overrides (Fuzzy Matched sum)
#         # Assuming override_map is {(DriverName, Date): Price}
#         driver_overrides = [v for (d, dt), v in override_map.items() if d == driver_name]
#         override_total = sum(driver_overrides)
#         ov_cell = ws.cell(row_idx, col_ptr)
#         ov_cell.value = override_total
#         if override_total > 0:
#             ov_cell.fill = OVERRIDE_FILL # Orange color
#         col_ptr += 1
...
#         tot_adp = f"{gcl('Total ADP Hours')}{row_num}"
...
#             f"(IF({w1_lds}<={sw_at}, {w1_lds}*350, ({sw_at}*350)+({tot_adp}/2)*{hr_rate})) + "
#             f"(IF({w2_lds}<={sw_at}, {w2_lds}*350, ({sw_at}*350)+({tot_adp}/2)*{hr_rate})), "
#             f"{tot_adp}*24))) + {ov_pay}"
#         )
...
#         # 7. Equivalent Hours & Adjustment
#         ws.cell(row_idx, col_ptr).value = f"={final_pay_col}{row_num}/24"
#         ws.cell(row_idx, col_ptr+1).value = f"={get_column_letter(col_ptr)}{row_num}-{tot_adp}"

#         # --- ANOMALY DETECTION (RED NAME) ---
#         # Logic: Relay Loads > 0 AND ADP Hours == 0
#         driver_cell = ws.cell(row_idx, 1)
#         relay_val = final_df.loc[row_idx-2, relay_cols].sum()
#         adp_val = final_df.loc[row_idx-2, adp_cols].sum()
        
#         if relay_val > 0 and adp_val == 0:
#             driver_cell.fill = ANOMALY_FILL # Red
#         else:
#             driver_cell.fill = DRIVER_FILL # Yellow

#         # Apply borders and center alignment to the new calc columns
#         for c in range(len(final_df.columns) + 1, col_ptr + 2):
